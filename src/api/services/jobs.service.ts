...
import { uploadFile, getFileUrl, minioClient } from '../../shared/services/storage.services.js';
...
// Helper para mapear la entidad de DB + el key de S3 al payload del worker
const mapJobToPayload = (job: JobEntity, objectName: string): JobPayload => ({
  jobId: job.id,
...
export const JobService = {
  async createAndEnqueue(file: Express.Multer.File, input: CreateJobInput) {
    ...
    try {
      ...
      // 4. Encolar job
      await addImageProcessingJob(mapJobToPayload(updatedJob, objectName));
